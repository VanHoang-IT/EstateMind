...
export default function AdminPropertiesPage() {
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-5 py-10 sm:px-7 lg:px-10">
      ...
      <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-[#e0e6e2] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <Search
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7e8983]"
          />
          <input
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Tìm kiếm tin đăng..."
            className="h-10 w-full rounded-lg border border-[#d7dfdb] pl-9 pr-3 text-sm outline-none transition focus:border-brand"
          />
        </form>
      </div>
        ...
        <div className="mt-6 flex flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#d7dfdb] text-[#4c5851] transition hover:border-brand hover:text-brand disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-[#d7dfdb] disabled:hover:text-[#4c5851]"
            >
              <ChevronLeft size={16} />
            </button>
            ...
            {buildPageList(page, totalPages).map((item, index) =>
              item === "…" ? (
                <span
                  key={`ellipsis-${index}`}
                  className="px-1.5 text-sm text-[#9aa39c]"
                >
                  …
                </span>
              ) : (
                <button
                  ...
                  className={`inline-flex h-9 min-w-[36px] items-center justify-center rounded-lg px-2 text-sm font-semibold transition ${
                    item === page
                      ? "bg-brand text-white"
