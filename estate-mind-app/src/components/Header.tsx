      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between text-sm">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex gap-6 font-medium text-gray-700">
            ...
            {user && (
              <Link href="/favorites" className="hover:text-red-500 transition-colors">
                Tin đã lưu
              </Link>
            )}
          </nav>
        ...
        <div className="flex items-center gap-4 text-gray-600 font-medium">
          {user ? (
            <>
              ...
              <button onClick={handleLogout} className="hover:text-red-500 hidden sm:block transition-colors">
                Đăng xuất
              </button>
            </>
          ) : (
            <>
              <Link href="/login" className="hover:text-red-500 hidden sm:block transition-colors">
                Đăng nhập
              </Link>
              <span className="hidden sm:block text-gray-300">|</span>
              <Link href="/register" className="hover:text-red-500 hidden sm:block transition-colors">
                Đăng ký
              </Link>
            </>
          )}
